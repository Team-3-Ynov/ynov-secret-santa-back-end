...
describe("email.service", () => {
  ...
  it("sendInvitationEmail should send email when SMTP succeeds", async () => {
    process.env.NODE_ENV = "production";
    process.env.SMTP_PORT = "587";
    process.env.SMTP_USER = "smtp-user";
    process.env.SMTP_PASS = "smtp-pass";
    sendMailMock.mockResolvedValueOnce({ messageId: "inv-1" });
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => undefined);
  ...
  it("sendDrawResultEmail should skip sending in production when creds are missing", async () => {
    process.env.NODE_ENV = "production";
    process.env.SMTP_PORT = "587";
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => undefined);
  ...
  it("sendDrawResultEmail should not throw in dev when send fails", async () => {
    process.env.NODE_ENV = "development";
    process.env.SMTP_PORT = "1025";
    sendMailMock.mockRejectedValueOnce(new Error("smtp down"));
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
