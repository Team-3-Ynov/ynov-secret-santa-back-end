...
vi.mock("../../src/services/event.service", () => ({
  createEvent: vi.fn().mockImplementation((payload) =>
    Promise.resolve({
      id: "uuid-1",
      title: payload.title,
  ...
  findEventById: vi.fn(),
  updateEvent: vi.fn().mockImplementation((id, payload) =>
    Promise.resolve({
      ...mockEvent,
      ...payload,
      id,
  ...
  createInvitation: vi.fn().mockImplementation((eventId, email) =>
    Promise.resolve({
      id: "invit-1",
      event_id: eventId,
      email: email,
...
describe("GET /api/events", () => {
  ...
  it("should return user events", async () => {
    ...
    const res = await request(app).get("/api/events").set("Accept", "application/json");
...
describe("PUT /api/events/:id", () => {
  ...
  it("should update an event successfully", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(eventService.updateEvent).toHaveBeenCalledWith(mockEvent.id, {
  ...
  it("should return 404 if event not found", async () => {
    ...
    const res = await request(app).put("/api/events/non-existent-id").send({ title: "New Title" });
  ...
  it("should return 403 if user is not the owner", async () => {
    ...
    const res = await request(app).put(`/api/events/${mockEvent.id}`).send({ title: "New Title" });
  ...
  it("should return 400 for invalid data", async () => {
    const res = await request(app).put(`/api/events/${mockEvent.id}`).send({ budget: -50 });
...
describe("DELETE /api/events/:id", () => {
  ...
  it("should return 404 if event not found", async () => {
    ...
    const res = await request(app).delete("/api/events/non-existent-id");
...
describe("DELETE /api/events/:id/invitations/:invitationId", () => {
  ...
  it("should delete a pending invitation successfully", async () => {
    ...
    const res = await request(app).delete(
      `/api/events/${mockEvent.id}/invitations/${mockInvitation.id}`
    );
  ...
  it("should return 404 if invitation not found", async () => {
    ...
    const res = await request(app).delete(`/api/events/${mockEvent.id}/invitations/non-existent`);
...
describe("POST /api/events/:id/join", () => {
  ...
  it("should join an event successfully", async () => {
    ...
    const res = await request(app).post(`/api/events/${mockEvent.id}/join`);

    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(notificationService.updateInvitationNotificationStatus).toHaveBeenCalledWith(
      "invit-1",
      1,
      "accepted"
    );
  ...
  it("should return 400 when invitation token is invalid", async () => {
    ...
    expect(res.status).toBe(400);
    expect(res.body.message).toBe("Aucune invitation trouvée pour cet événement.");
    expect(eventService.joinEvent).toHaveBeenCalledWith(mockEvent.id, 1, "bad-token");
  });
...
describe("PATCH /api/events/:id/invitations/:invitationId/decline", () => {
  ...
  it("should decline an invitation successfully", async () => {
    ...
    const res = await request(app).patch(`/api/events/${mockEvent.id}/invitations/invit-1/decline`);
  ...
  it("should return 404 when invitation does not exist", async () => {
    ...
    const res = await request(app).patch(
      `/api/events/${mockEvent.id}/invitations/non-existent/decline`
    );
...
describe("POST /api/events/:id/draw", () => {
  ...
  it("should perform draw successfully", async () => {
    ...
    const res = await request(app).post(`/api/events/${mockEvent.id}/draw`);
  ...
  it("should return 404 if event not found", async () => {
    ...
    const res = await request(app).post("/api/events/non-existent/draw");
